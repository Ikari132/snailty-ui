...
const getTsConfigAliases = (config) => {
    const paths = config?.compilerOptions?.paths;
    if (!paths) {
        return {}
    }
    const aliases =
        Object.keys(paths).reduce((alias, key) => {
            const aliasPath = Array.isArray(paths[key]) ? paths[key][0] : paths[key];

            alias.push({
                find: key.replace("*", ""), replacement: path.resolve(__dirname, aliasPath).replace("*", "/")
            })
